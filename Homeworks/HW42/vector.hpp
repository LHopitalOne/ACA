...
public:
    ...
    _ValueT& at(std::size_t index) const;
...
// template<typename _ValueT>
// void vector<_ValueT>::resize(std::size_t n)
// {
//     if (size() > n)
//     {
...
template <typename _ValueT>
inline _ValueT &vector<_ValueT>::at(std::size_t index) const
{
    if (index < mSize)
        return mData[internal_index(index)];
    throw exception{"Error: trying to access element out of the array."};
}
...
void vector<_ValueT>::resize(std::size_t newCapacity)
{
    ...
    for (std::size_t i = 0; i < (newCapacity >= mSize ? mSize : newCapacity); i++)
        newData[i] = mData[(front_index + i) % mCapacity];
    
    if (mSize < newCapacity)
        for (std::size_t i = mSize; i < newCapacity; i++)
            newData[i] = _ValueT();
    ...
    front_index    = 0;
    back_index     = newCapacity;
}

template<typename _ValueT>
void vector<_ValueT>::reserve(std::size_t newCapacity)
{
